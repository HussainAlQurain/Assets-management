...
mobiles.config['SECRET_KEY'] = 'COiKOfipSydA4msx5QdGuHEQYjBlpH2h'
...
def main():
    ...
    for row in cursor.fetchall():
        mobiles.append({"id": row[0], "name": row[1], "model": row[2], "color": row[3], "empID": row[4]})
    conn.close()
...
def updatemobile(id):
    ...
    if request.method == 'GET':
        ...
        for row in cursor.fetchall():
            mobile.append({"id": row[0], "name": row[1], "model": row[2], "color": row[3], "empID": row[4]})
        conn.close()
    ...
    if request.method == 'POST':
        ...
        cursor.execute("UPDATE dbo.Mobile SET name = ?, model = ?, color = ?, empID = ? WHERE id = ?", name, model, color, empID, id)
        conn.commit()
        conn.close()
...
def addmobile():
    ...
    if request.method == 'POST':
        ...
        model = str(request.form["model"])
        color = str(request.form["color"])
        cursor.execute("INSERT INTO dbo.Mobile VALUES (?, ?, ?, ?)", name, model, color, empID)
        conn.commit()
        conn.close()
        return redirect('/Mobiles')
...
def addcomputer():
    ...
    if request.method == 'GET':
        ...
        for row in cursor.fetchall():
            computer.append({"id": row[0], "Office": row[1], "Department": row[2], "User": row[3], "Computer name": row[4], "Computer Brand": row[5], "Computer Model No": row[6], "Service tag / Serial No": row[7], "OS": row[8], "CPUs": row[9], "Ram": row[10], "HD Size": row[11], "HD Type": row[12], "Office365": row[13], "Adobe Pro": row[14], "Microsoft Projects": row[15], "Visio": row[16], "Brand": row[17], "Size": row[18], "Travel port DS": row[19], "External HD": row[20], "Bag": row[21], "History": row[22], "empID": row[23]})
        conn.close()
        return render_template("updatecomputer.html", computer = [])
    if request.method == 'POST':
        Office = str(request.form["Office"])
        Department = str(request.form["Department"])
        User = str(request.form["User"])
...
def searchby():
    ...
    if request.method == 'POST':
        ...
        if selected == "userID":
            ...
            for row in cursor.fetchall():
                assets.append({"Office": row[1], "Department": row[2], "Computer name": row[4], "Computer Brand": row[5], "Computer Model No": row[6], "Service tag / Serial No": row[7], "OS": row[8], "CPUs": row[9], "Ram": row[10], "HD Size": row[11], "HD Type": row[12], "Office365": row[13], "Adobe Pro": row[14], "Microsoft Projects": row[15], "Visio": row[16], "Brand": row[17], "Size": row[18], "Travel port DS": row[19], "External HD": row[20], "Bag": row[21], "History": row[22]})
...
if(__name__ == "__main__"):
    mobiles.secret_key = 'COiKOfipSydA4msx5QdGuHEQYjBlpH2h'
